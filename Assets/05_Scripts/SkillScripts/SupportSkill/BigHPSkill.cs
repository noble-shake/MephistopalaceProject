...
public class BigHPSkill : ISkill
{
    ...
    public BigHPSkill(PlayerManager playerManager)
    {
    ...
    public void Execute()
    {
        EventMessageManager.Instance.MessageQueueRegistry(new EventContainer() { eventType = ContextType.Battle, Context = "대형 HP 물약을 사용합니다." });
    ...
    public void Process(ProcessType _React)
    {
        foreach (BattlePhase target in playerManager.battler.CurrentTargets)
        {
            ...
            InventoryManager.Instance.ItemUse(SkillActions.BigHPConsume, playerPhase.playerManager); // 여기까지 온 이상, 인벤토리 안에 해당 아이템이 없을 이유가 없다.
        }
