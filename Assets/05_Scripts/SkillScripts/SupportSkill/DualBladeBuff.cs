...
public class DualBladeBuff : ISkill
{
    ...
    public DualBladeBuff(PlayerManager playerManager)
    {
    ...
    public void Execute()
    {
        EventMessageManager.Instance.MessageQueueRegistry(new EventContainer() { eventType = ContextType.Battle, Context = "(쌍검사) 전투의 함성을 사용합니다." });
    ...
    public void Process(ProcessType _React)
    {
        foreach (BattlePhase target in playerManager.battler.CurrentTargets)
        {
            ...
            playerPhase.playerManager.status.GainAP(1);
        }
